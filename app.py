...
* **손절 (SL):** 매우 직관적이고 칼같은 손절이 가능합니다. **오더블록 박스의 반대쪽 끝(꼬리 끝)**을 이탈하여 '봉 마감'이 될 때가 완벽한 손절 라인입니다."""
...
* **S/R Flip (지지/저항 반전):**
    ...
    * **Inverse FVG:** 만약 캔들이 FVG 영역을 완전히 돌파(봉 마감)해버린다면, 성질이 반대로 바뀝니다. (지지가 저항으로, 저항이 지지로) 이후 리테스트 시 매우 좋은 진입 타점이 됩니다.
...
* **흔한 오해:** 많은 초보자들이 차트 패턴을 단순히 과거에 나타났던 '모양'으로만 암기하려고 합니다.
...
* **컵(Cup):** 고점에서 물린 개미들의 실망 매물이 나오며 가격이 천천히 하락합니다. (U자형 바닥). 바닥 구간에서 세력은 가격을 올리지 않고 조용히, 그리고 길게 물량을 매집합니다. 이때 거래량은 바짝 마릅니다.
* **핸들(Handle):** 가격이 전고점에 다다르면, 개미들의 **'본전 심리'**가 발동하여 강력한 매도 물량이 쏟아집니다. 세력이 이 물량을 다 받아먹으면서 가격이 살짝 눌리게 되며, 이것이 손잡이(핸들) 모양이 됩니다.
...
**■ 3. 컵 앤 핸들의 3가지 핵심 조건**
1. **형태:** 컵의 바닥은 뾰족한 V자가 아니라 **둥근 U자형**이어야 합니다. (충분한 매집 시간이 필요함)
2. **비율:** 핸들의 깊이(눌림)는 컵 전체 높이의 절반(50%)을 넘지 않는 것이 좋습니다. **이상적인 눌림 폭은 30% 내외**입니다.
3. **거래량 (가장 중요!):** 컵 바닥과 핸들 구간에서는 반드시 **거래량이 바짝 말라야** 합니다. 그리고 마지막 저항선을 돌파할 때는 폭발적인 거래량이 터져야 신뢰도가 높습니다."""
...
* **형태:** 이름 그대로 마름모(다이아몬드) 모양을 띠며, 두 가지 상반된 패턴이 결합된 형태입니다.
    1. **전반부 (확산형 패턴):** 고점은 높아지고 저점은 낮아지며 가격 변동폭 확대.
    2. **후반부 (수렴형 패턴):** 고점은 낮아지고 저점은 높아지며 가격 변동폭 축소.
...
* **다이아몬드 탑 (Diamond Top):** 긴 상승 추세 끝 발생 -> **강력한 하락 반전**.
* **다이아몬드 바텀 (Diamond Bottom):** 긴 하락 추세 끝 발생 -> **강력한 상승 반전**.

**■ 3. 거래량 힌트 (중요)**
* 전반부(확산형)에서는 매수/매도 싸움으로 **거래량이 높게 튀는 현상**이 발생.
* 후반부(수렴형)로 갈수록 점차 **거래량이 말라감**. 마지막 경계선 돌파 시 다시 거래량 크게 폭발."""
...
* **아담 (Adam - V자형 바닥):** 극도의 공포와 패닉 셀(투매)이 나오며 세력이 이를 급하게 받아먹어 **V자 급반등**이 나옵니다. 이때 **거래량이 크게 터집니다.**
* **이브 (Eve - U자형 바닥):** 아담 이후 다시 하락하지만 완만한 **U자(접시) 모양**을 만듭니다. 아담에서 털리지 않은 개미들을 서서히 지치게 만들며 세력이 조용히 매집하는 구간이므로 **거래량이 크게 줄어듭니다.**
...
**■ 3. 아담 앤 이브의 핵심 확인 조건**
1. **순서:** 반드시 날카로운 V자(아담)가 먼저 나오고, 그 다음에 완만한 U자(이브)가 나와야 합니다.
2. **깊이:** 이브(U)의 저점은 아담(V)의 저점과 비슷하거나 살짝 높은 것이 안전합니다.
3. **진입:** 두 골짜기 사이의 중앙 고점(넥라인)을 돌파하거나, 돌파 후 리테스트할 때 진입합니다."""
...
* Fake out은 따라잡기 힘들지만, Trap은 완벽한 진입 찬스를 제공합니다. (숏 투자자들을 한 번 더 꼬시는 '데드캣 바운스' 움직임 이후 물량을 최종 매집하는 형태입니다.)
* **진입 트리거:** 박스권 하향 돌파 실패 후 재진입 -> **두 번째 바닥(쌍바닥 지지) 확인 시** 진입.
* **손절 (SL):** 두 번째 바닥을 지지하지 못하고 첫 번째 바닥의 꼬리 끝을 다시 이탈 마감할 때 칼같이 손절."""
...
* **[1단계] 상위 타임프레임 (일봉 1D / 주봉 1W) - "숲의 방향 파악"**
    * 전체적인 시장의 큰 추세와 거대한 오더블록, 강력한 지지/저항선을 파악합니다.
* **[2단계] 중위 타임프레임 (4시간봉 4H / 1시간봉 1H) - "작전 반경 파악"**
    * 상위 프레임의 틀 안에서 현재 형성 중인 채널, 가까운 유동성, 단기 FVG 등을 확인합니다.
* **[3단계] 하위 타임프레임 (15분봉 15m / 5분봉 5m) - "정밀 타격(Entry)"**
    * 상위 추세와 일치하는 방향으로만! 5분봉의 유동성 스윕이나 패턴 돌파 시 방아쇠를 당깁니다.
...
* **초보자들의 흔한 착각:** 보조 지표(RSI, MACD, 볼린저 밴드, 이평선 등)를 많이 띄워놓고 선을 이리저리 많이 그어두면 '전문적인 분석'을 한다고 착각합니다.
* 지표가 충돌하면 결국 뇌동매매에 빠지거나 훌륭한 타점을 놓칩니다.

**■ 2. 핵심 원칙: "필요한 것만 남기고 다 지우자"**
* 진짜 고수들의 차트는 놀라울 정도로 단순하고 깨끗합니다.
* 가격(Price)과 거래량(Volume)이라는 가장 본질적인 데이터에 집중하세요.
* 과거의 모든 고점에 선을 긋지 말고, **현재 가격에 직접 영향을 미치는 가장 최근의 핵심 구조물(오더블록, FVG, 유동성 스윕 라인)** 몇 개만 남기고 과감히 지우세요."""

    rr_ratio_text = """**■ 1. 손익비(Risk/Reward Ratio)란 무엇인가?**
* **개념:** 한 번의 매매에서 **'감수해야 할 손실 금액(Risk)'** 대비 **'얻을 수 있는 예상 수익 금액(Reward)'**의 비율.
...
진입가: $100
손절가: $90 (리스크 $10)
목표가: $130 (수익 $30) -> 1:3 손익비 달성!

10번 매매해서 7번 잃고 3번만 이겨도?
(3승 * $30) - (7패 * $10) = $90 - $70 = +$20 수익!
```

**■ 2. 왜 손익비가 승률보다 중요한가?**
* 승률 90%라도 1번 크게 지면 청산당하는 것이 시장입니다.
* 높은 손익비는 나의 낮은 승률(잦은 손절)을 완벽하게 커버해주는 **가장 강력한 생명줄**입니다.
* 진입하려는 타점의 손익비가 최소 **1:2 이상**이 나오지 않는다면 아무리 예뻐 보여도 과감하게 매매를 포기(Pass)해야 합니다."""

    stop_loss_text = """**■ 1. 손절은 패배가 아닌 '생존'이자 '사업 비용'**
* **자본 보존 (시드 지키기):** 잃지 않아야 다음 기회에 배팅할 수 있습니다. 단 한 번의 큰 손실이 그동안 쌓아온 모든 수익을 날릴 수 있습니다.

```text
[ 손실 복구의 수학적 한계 ]
-10% 손실 -> +11% 수익 필요
-30% 손실 -> +43% 수익 필요
-50% 손실 -> +100% 수익 필요 (원금 반토막 나면 2배를 벌어야 함!)
```

**■ 2. 기계적인 손절을 위한 실전 팁**
* 감정이 개입하기 전인 '진입 계획 단계'에서 무조건 손절가(SL)를 정해야 합니다.
* 진입과 동시에 거래소 시스템에 **스탑로스(Stop-Market)를 즉시 세팅**하세요.
* 손절을 '내 돈 잃는 것'이 아니라 트레이딩 사업의 '임대료/유지비'로 인식하세요."""

    position_sizing_text = """**■ 1. 비중 조절(Position Sizing)이란?**
* "얼마나 살 것인가?" 아무리 완벽한 타점과 칼같은 손절(SL)을 준비했어도, 몰빵(과도한 비중)을 했다면 한 번의 휩소에 계좌가 파산합니다.

**■ 2. 핵심 원칙: '고정 리스크(Fixed Risk)' 매매**
* 한 번의 매매에서 전체 시드(자본금)의 **고정된 비율(보통 1~2%)**만 잃도록 진입 수량을 조절하는 방식입니다.
* **생존의 마법:** 1% 고정 리스크를 사용하면 연속으로 10번을 패배(손절)해도 전체 시드의 약 10%만 줄어들 뿐, 여전히 복구할 강력한 힘이 남습니다.

**■ 3. 안전 진입 수량 계산 공식**
* `허용 가능한 최대 손실 금액 (시드의 1%)` ÷ `1개당 예상 손실폭 (진입가 - 손절가)` = **안전 진입 수량**
* 👉 본 대시보드의 **[🤖 자동매매 사령실 -> 🛡️ 반자동 생존 매매]** 탭에 이 공식이 시스템으로 자동 탑재되어 있습니다."""

    s_class_setup_text = """**■ 1. 셋업(Setup)의 본질**
* 유동성, 추세선, 오더블록, 패턴 등은 '무기 상점'에 진열된 다양한 무기들일 뿐입니다.
* **핵심:** 이 무기들을 실전에 적용해보고, **'내 성향에 가장 잘 맞고 손에 익는 무기'**를 선별해내는 과정이 트레이딩의 첫걸음입니다.

**■ 2. S급 셋업 구축 방법 (조합의 마법 - Confluence)**
* 단일 근거로 진입하는 것은 승률이 낮습니다. 여러 개의 기술적 분석 근거가 '중첩'되는 자리를 찾아야 합니다.

```text
[ S급 타점 중첩 예시 ]
✅ 1. 1시간봉 주요 전저점 유동성 스윕 발생
✅ 2. 동시에 15분봉 상승 오더블록(Bullish OB) 형성
✅ 3. 해당 자리가 일봉 상승 채널 하단 지지선과 겹침
🎯 -> 근거 3개가 겹치는 완벽한 S급 셋업! (비중 상향 베팅 가능)
```

**■ 3. 트레이딩 성장의 비밀: 철저한 '매매 일지' 작성**
* 본 대시보드의 **[📝 매매 기록 보관지]** 탭을 활용하여 진입/종료 근거와 차트 캡처를 기록하세요.
* 내 승률이 높은 S급 셋업만 남기고 나머지는 버리는 것이 궁극의 생존 비결입니다."""
