...
* **의미:** 차트에서 매우 드물게 나타나지만, 한 번 출현하면 **추세의 전환(반전)**을 알리는 아주 강력 신호입니다. 
...
* **익절 (TP):** 최소 목표가는 넥라인에서부터 **'아담(V자)의 바닥까지의 깊이'**만큼 위로 1:1 대칭하여 설정합니다."""
    ...
    # 💡 9. 거짓 돌파(Fake out / Whipsaw) 패턴 (신규 업데이트)
    fakeout_text = """**■ 1. 거짓 돌파(Fake out)의 본질**
* **의미:** 가격이 중요한 지지선이나 저항선을 돌파한 것처럼 보였으나, 이내 방향을 급격히 틀어 **원래 자리로 되돌아오는 현상**입니다. (흔히 '휩소(Whipsaw)', '트랩(Trap)', '불/베어 트랩'으로 불립니다.)
* **핵심:** 이것은 단순한 '실패'가 아니라, 세력이 개미들의 **유동성을 흡수하기 위해 고의로 판 함정**입니다. 

**■ 2. 세력은 왜 거짓 돌파를 만드는가? (심리전)**
세력은 막대한 자금을 운용하기 때문에, 큰 포지션을 진입하려면 그 반대편의 물량(개미들의 손절 물량)이 대량으로 필요합니다.
* **돌파 매수 꼬시기:** 저항선을 뚫어주면 수많은 개미들이 "아! 드디어 돌파다!" 하고 추격 매수(롱)를 따라붙습니다.
* **손절 물량 받아먹기:** 저항선 위에는 기존에 숏(매도)을 쳤던 개미들의 손절(SL=매수) 물량이 잔뜩 쌓여 있습니다.
* **수확:** 세력은 이 두 가지 매수 물량(추격 매수 + 숏 손절 물량)을 받아내며 자신들의 **대규모 숏(매도) 포지션을 체결**시키고, 곧바로 가격을 아래로 패대기칩니다.

**■ 3. 🎯 거짓 돌파 확인 및 진입 셋업**
거짓 돌파에 속아서 손절만 하던 피해자에서 벗어나, 이 트랩을 오히려 **역이용하는 타점**을 잡아야 합니다.

* **확인 조건 (가장 중요):**
    1. 중요 레벨(저항/지지선)을 강하게 뚫고 나감.
    2. **봉 마감을 관찰!** 뚫었던 캔들이 꼬리를 달고 다시 선 안쪽으로 들어와서 마감하거나, 그다음 캔들이 즉시 선 안쪽으로 다시 기어 들어올 때. (Fake out 확정)
* **진입 트리거 (Entry):**
    * 가격이 선 안쪽으로 다시 들어와서 마감하는 순간, **거짓 돌파의 '반대 방향'**으로 진입합니다. 
    * *(예: 저항선 상향 돌파 후 다시 내려왔다면 숏(매도) 진입)*
* **손절 (SL):**
    * 거짓 돌파를 만들면서 찔렀던 **가장 긴 꼬리의 끝점**입니다. (만약 이 선이 다시 돌파된다면 진짜 추세가 터진 것이므로 빠르게 도망쳐야 합니다.)
* **익절 (TP):**
    * 세력은 유동성을 확보했으므로 반대편 유동성(다음 지지/저항선)을 향해 강하게 밀어붙일 확률이 높습니다. 다음 주요 매물대나 반대편 채널 선을 1차 익절 라인으로 잡습니다."""
    ...
    db_dict = {
        ...
        "3. 거짓 돌파 (트랩)": {
            "거짓 돌파(Fake out)의 모든 것": {"id": "default", "content": fakeout_text, "images": []}
        },
        "4. 심화 (하모닉 패턴)": {
            ...
            if "GEMINI_API_KEY" not in st.secrets:
                ...
                with st.spinner('AI가 유동성 스윕 여부와 셋업을 정밀 분석 중입니다... 🤖'):
                    try:
                        ...
                        1. 레벨 식별: 차트상 주요 전고점/전저점 등 유동성이 몰려있는 구간 파 파악
