...
with tab1:
    ...
    with st.expander("➕ 새로운 매매 기록 추가하기", expanded=True):
        ...
        with st.container(border=True):
            ...
            with col2: 
                ticker = st.text_input("종목명 (예: BTC)").upper()
            with col3: 
                timeframe = st.selectbox("타임프레임", ["1m", "5m", "15m", "1H", "4H", "1D"])
            with col4: 
                setup_pattern = st.text_input("셋업/패턴")
        ...
        with st.container(border=True):
            ...
            with col7: 
                margin = st.number_input("투자 원금 ($)", min_value=0.0, value=1000.0, step=100.0)
            with col8: 
                entry_price = st.number_input("진입 가격", min_value=0.0, value=0.0, format="%.4f")
            with col9: 
                exit_price = st.number_input("종료 가격", min_value=0.0, value=0.0, format="%.4f")
        ...
        with st.container(border=True):
            ...
            with col10: 
                result = st.selectbox("최종 결과", ["승", "무", "패"], index=idx_res)
            with col11: 
                rr_ratio = st.text_input("손익비 (예: 1:2)")
    ...
    if not df_trade.empty:
        ...
        if selected_event.get('selection', {}).get('rows', []):
            ...
            with col_t: 
                st.markdown(f"## 🧐 {trade_data['date']} | {trade_data['ticker']} 복기")
            with col_d:
            ...
            with c_chart:
                for u in str(trade_data.get("chart_image_paths", "")).split("|"):
                    if u: 
                        st.markdown(render_crisp_image_html(u), unsafe_allow_html=True)
            with c_memo:
    ...
    with col2:
        ...
        if st.button("🚀 아카이브 기반 AI 관점 분석 요청", type="primary", use_container_width=True):
            ...
            if not keys: 
                ...
                st.warning("⚠️ 차트 이미지와 티커를 입력해주세요.")
    ...
    if st.session_state.ai_analysis_done:
        ...
        with st.expander("💾 이 관점을 '나의 관점(Watchlist)'에 저장하기", expanded=True):
            with st.form("save_watchlist_form"):
                ...
                with col_w1: 
                    w_ticker = st.text_input("종목명", value=ticker_input).upper()
                with col_w2: 
                ...
                if st.form_submit_button("☁️ 클라우드 저장", type="primary"):
                    if th_title and th_cont:
                        ...
                        insert_db("theory_db", {
                            ...
                            "title": th_title, 
                            "content": th_cont, 
                            "image_paths": "|".join([u for u in img_urls if u])
                        })
                        st.rerun()
...
with tab4:
    ...
    with bot_tab2:
        with st.form("survival_trade_form"):
            ...
            with col_s1: 
                sv_symbol = st.text_input("종목명", value="BTC/USDT:USDT")
            with col_s2: 
                sv_side = st.selectbox("포지션", ["buy (Long)", "sell (Short)"])
            with col_s3: 
                sv_sl_percent = st.number_input("손절 비율 (%)", 0.1, 10.0, 2.0, 0.1)
            ...
            if st.form_submit_button("🚀 진입 및 스탑로스 자동 세팅", type="primary", use_container_width=True):
                if not st.session_state.get('bg_api'): 
                    st.error("API 키를 저장해주세요.")
                else:
                    success, msg = execute_survival_trade(
                        st.session_state['bg_api'], 
                        st.session_state['bg_secret'], 
                        st.session_state['bg_pass'], 
                        sv_symbol, 
                        "buy" if "buy" in sv_side else "sell", 
                        sv_sl_percent, 
                        sv_reason, 
    ...
    with bot_tab3:
        st.code("[https://youngwoo-trading.streamlit.app/webhook](https://youngwoo-trading.streamlit.app/webhook)", language="text")
    with bot_tab4:
    ...
    with sub_tab_a:
        with st.expander("➕ 새로운 스크랩 추가하기", expanded=False):
            with st.form("archive_form_others", clear_on_submit=True):
                ...
                with col1: 
                    arch_date1 = st.date_input("스크랩 날짜", datetime.today())
                with col2: 
                    arch_ticker1 = st.text_input("종목명").upper()
                with col3: 
                    arch_source1 = st.text_input("출처/제목")
                ...
                if st.form_submit_button("☁️ 스크랩 & 무료 AI 분석 시작", type="primary"):
                    if not arch_ticker1: 
                        st.error("관련 종목명을 최소 1개 이상 입력해주세요!")
                    else:
                        with st.spinner("AI 분석 및 클라우드 저장 중..."):
                            ...
                            insert_db("analysis_archive", {
                                "date": arch_date1.strftime("%Y-%m-%d"), 
                                "ticker": arch_ticker1, 
                                "category": "타인분석", 
                                "source_view": arch_source1, 
                                "chart_image_paths": "|".join(blog_urls), 
                                "detail_image_paths": "|".join(detail_urls), 
                                "memo": "", 
                                "ai_advice_mapping": json.dumps(ai_map, ensure_ascii=False), 
                                "ocr_text_mapping": json.dumps(ocr_map, ensure_ascii=False)
                            })
                            st.success("완료!")
                            time.sleep(1)
                            st.rerun()
        ...
        if not df_others.empty:
            ...
            if selected_other.get('selection', {}).get('rows', []):
                ...
                try: 
                    ocr_map = json.loads(arch_data.get("ocr_text_mapping", "{}"))
                except: 
                    ocr_map = {}
                try: 
                    ai_map = json.loads(arch_data.get("ai_advice_mapping", "{}"))
                except: 
                    ai_map = {}
                ...
                for idx, path in enumerate(blog_paths):
                    ...
                    if group in ocr_map and ocr_map[group]: 
                        st.info(ocr_map[group])
                ...
                for idx, path in enumerate(detail_paths):
                    ...
                    with c_ai:
                        ...
                        if key in ai_map and ai_map[key]: 
                            render_ai_advice_block("🤖 AI 조언", ai_map[key])
                        else: 
                            st.write("AI 분석 결과 없음")
    ...
    with sub_tab_b:
        ...
        if not df_my.empty:
            ...
            if selected_my.get('selection', {}).get('rows', []):
                ...
                st.info(f"💡 나의 관점:\n\n{my_data['source_view']}")
                if my_data.get('memo'): 
                    render_ai_advice_block("🤖 AI 피드백", my_data['memo'])

# ==========================================
# --- Tab 6: 섹터 & 주도주 맵 (🚨 분리된 4개의 모듈 탑재) ---
# ==========================================
with tab6:
