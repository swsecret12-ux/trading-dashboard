...
def render_kr_map_tab():
    ...
    if not st.session_state.kospi100_state_df.empty:
        ...
        try:
            ...
            end_date_fetch = datetime.now() + timedelta(days=2) # 금요일 데이터 누락 방지를 위해 미래 날짜로 넉넉히 설정
            start_date_fetch = end_date_fetch - timedelta(days=1470) # 약 4년 전
            
            for _ in range(3):
                try:
                    temp_df = yf.download("^KS11", start=start_date_fetch.strftime('%Y-%m-%d'), end=end_date_fetch.strftime('%Y-%m-%d'), interval="1d", progress=False)
        ...
        try:
            ...
            # 💡 금요일 데이터 누락 방지 로직 (주말 캐시 우회)
            end_date_chart = datetime.now() + timedelta(days=2)
            start_date_chart = end_date_chart - timedelta(days=365)
            
            for _ in range(3):
                try:
                    kospi_daily = yf.download("^KS11", start=start_date_chart.strftime('%Y-%m-%d'), end=end_date_chart.strftime('%Y-%m-%d'), interval="1d", progress=False)
