...
def analyze_sector_with_ai(ticker, sector, fin_data, user_input="", saveticker_text=""):
    ...
    **[중요 지시사항]: 보고서의 분량을 기존보다 3배 이상 길고 심층적으로 작성하십시오. 단순 숫자 나열은 금지하며, 아래 4가지 항목을 매우 깊이 있게 분석하세요.**
    ...
    1. 🏢 시장 위치 및 밸류체인 심층 분석
       ...
       - 주요 경쟁사 및 연계된 공급망(밸류체인) 주식들과의 커플링/디커플링 동향을 심층 분석.
    2. 🌍 매크로 동향 및 핵심 뉴스 팩트체크 (가장 중요)
       - 제공된 뉴스(특히 대형 기업과의 파트너십, M&A 등)가 펀더멘털에 미치는 실제 파급력을 분석. (예: SNOW-아마존 협업 뉴스가 있다면 그 이유와 향후 매출 기여도 전망 등).
       - 현재 금리나 섹터 전반의 매크로 환경이 이 종목에 미치는 영향 서술.
    3. 💰 실적(Earnings) 종합 의견
       - 최근 1~2개 분기의 실적 상회/하회 여부가 시장에 어떤 시그널을 주었는지, 그리고 다음 가이던스에 대한 기관들의 시각을 추론.
    4. 💡 기관 트레이딩 결론 (Actionable Insight)
       - 펀더멘털과 뉴스 모멘텀을 종합하여, 현재 포지션 진입(신규 Long/Short/관망)에 대한 구체적이고 냉철한 조언.
    """
